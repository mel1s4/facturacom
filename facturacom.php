...
function form_shortcode(){
  return FacturaWrapper::form_shortcode();
}
...
function custom_add_order_link($actions, $order) {
  ...
  if($billing_email !== '') {
    $query .= '&email=' . $billing_email;
  }
  $url = '/facturacom?' . $query;
  $actions['custom_facturacion'] = array(
      'url'    => $url,
      'name'   => __('Facturación', 'custom-order-link'),
      'action' => "window.location.href='$url'",
  );

  return $actions;
}
add_filter('woocommerce_order_actions', 'custom_add_order_link', 10, 2);

function custom_add_order_column($columns) {
    $new_columns = array();
    ...
    foreach ($columns as $key => $column) {
        ...
        if ($key === 'order_status') {
            $new_columns['custom_facturacion'] = __('Facturación', 'custom-order-link');
        }
    }

    return $new_columns;
}
add_filter('manage_edit-shop_order_columns', 'custom_add_order_column', 20);

function custom_populate_order_column($column, $post_id) {
    if ($column === 'custom_facturacion') {
      $order = wc_get_order($post_id);
      $user_id = $order->get_user_id();
      $order_id = $order->get_id();
      $billing_rfc = get_post_meta($order_id, '_billing_rfc', true);
      $billing_email = $order->get_billing_email();
      $query .= 'order=' . $order_id;
		  if($billing_email !== '') {
			  $query .= '&email=' . $billing_email;
		  ...
		  if($$billing_rfc !== '') {
			  $query .= '&rfc=' . $$billing_rfc;
		  }
      $url = '/facturacom?' . $query;

    echo '<a href="' . esc_url($url) . '">' . __('Facturación', 'custom-order-link') . '</a>';
  }
}
add_action('manage_shop_order_posts_custom_column', 'custom_populate_order_column', 10, 2);


// Add the custom billing field to the checkout page
function add_custom_billing_field($checkout_fields) {
    $checkout_fields['billing']['billing_rfc'] = array(
        'type'        => 'text',
        'class'       => array('form-row-wide'),
        'label'       => __('RFC', 'custom-billing-fields'),
        'placeholder' => __('Your RFC', 'custom-billing-fields'),
        'required'    => true,
    );
...
// Save the custom billing field to the order
function save_custom_billing_field($order_id) {
    $billing_rfc = sanitize_text_field($_POST['billing_rfc']);
    update_post_meta($order_id, '_billing_rfc', $billing_rfc);
}
add_action('woocommerce_checkout_update_order_meta', 'save_custom_billing_field');

// Display and edit the custom billing field in My Account
function display_and_edit_custom_billing_field() {
    $user_id = get_current_user_id();
    $billing_rfc = get_user_meta($user_id, '_billing_rfc', true);
?>
    <p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
        <label for="billing_rfc"><?php _e('RFC (Registro Federal de Contribuyentes)', 'custom-billing-fields'); ?>:</label>
        <input type="text" class="woocommerce-Input woocommerce-Input--text input-text" name="billing_rfc" id="billing_rfc" value="<?php echo esc_attr($billing_rfc); ?>" />
    </p>
<?php
}
add_action('woocommerce_edit_account_form', 'display_and_edit_custom_billing_field');

// Save changes to the custom billing field in My Account
function save_custom_billing_field_in_my_account() {
    if (isset($_POST['billing_rfc'])) {
        $user_id = get_current_user_id();
        $billing_rfc = sanitize_text_field($_POST['billing_rfc']);
        update_user_meta($user_id, '_billing_rfc', $billing_rfc);
    }
}
add_action('woocommerce_save_account_details', 'save_custom_billing_field_in_my_account');
